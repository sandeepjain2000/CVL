...
def _career_inbox_unprocessed_uids(mail: imaplib.IMAP4_SSL) -> set:
    """
    INBOX UIDs in the lookback window that do not yet have PROCESSED-CAREER.
    ...
    logger.info(
        ...
        f"excluding label {GMAIL_LABEL_PROCESSED!r} (X-GM-LABELS)…"
    )
    return _inbox_uids_since_without_processed_label(
        mail, LOOKBACK_DAYS, GMAIL_LABEL_PROCESSED
    )
    ...
    while i < len(career_uids):
        ...
        try:
            ...
            if forward_count >= MAX_FORWARDS_PER_ACCOUNT:
                logger.info(
                    f"  {ap}🛑 Forward limit ({MAX_FORWARDS_PER_ACCOUNT}) — "
                    "remaining career UIDs left without PROCESSED-CAREER for next run."
                )
                break
            ...
            if not is_career_related(orig_subject, combined):
                logger.info(f"  {ap}⏭️  Skipped non-career: {orig_subject}")
                log_processed(
                    conn, execution_id, gmail_address,
            ...
            if smtp_conn is None:
                logger.warning(
                    f"  ⚠️  No SMTP — cannot forward: {orig_subject}"
                )
                stats["errors"] += 1
                i += 1
                continue
            ...
            logger.info(
                f"  {ap}➔  Forwarding (REPLY-CAREER → {TARGET_FORWARD_EMAIL}): "
            ...
            forward_count += 1
            _tag_career_message(mail, uid, GMAIL_LABEL_REPLY)
