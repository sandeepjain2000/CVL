...
def check_inbox(gmail_address: str, app_password: str,
    ...
    try:
        ...
        while i < len(ids_list):
            msg_id = ids_list[i]
            try:
                # Keep-alive ping every 50 messages to prevent idle timeout
                if i > 0 and i % 50 == 0:
                    try:
                        mail.noop()
                    except Exception:
                ...
                # deleted/flagged messages — data[0][1] would be an int not bytes
                ...
                if reconnects >= 3:
                    logger.warning(f"  Too many reconnects; skipping message {msg_id}: {e}")
                    i += 1
                    continue
                reconnects += 1
